...
public class UniCLI implements CLI {
    ...
    private void evaluationOperations(Scanner scanner, CRUDRepository<Evaluation> repository) {
        ...
        switch (operation) {
            case 1:
                ...
                Evaluation evaluation = CreateEvaluation.createEvaluation(subjectName, evalName, studentName, evalType);
    ...
    public <T extends Entity> T findEntityById(CRUDRepository<T> repository, int id) {
        ...
        if (entity == null) {
            throw new EntityException("Entity with ID " + id + " not found.");
        }
            return entity;
        }
}